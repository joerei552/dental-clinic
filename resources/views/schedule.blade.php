...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Dental Clinic - Schedule</title>
    ...
    <body class="antialiased bg-gray-50 dark:bg-gray-900">
        ...
        <div class="fixed inset-y-0 left-0 w-64 bg-white dark:bg-gray-800 shadow-lg">
            ...
            <nav class="mt-6">
                <div class="px-4 space-y-2">
                    ...
                    <a href="{{ route('schedule') }}" class="flex items-center px-4 py-2 text-gray-700 bg-gray-100 rounded-lg dark:bg-gray-700 dark:text-gray-200">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="ml-64 p-8">
            ...
            <div class="flex justify-between items-center mb-8">
                ...
                <div class="flex space-x-4">
                    <button onclick="previousWeek()" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200">
                        Previous Week
                    </button>
                    <button onclick="nextWeek()" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200">
                        Next Week
                    </button>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                ...
                <div class="grid grid-cols-8 gap-px bg-gray-200 dark:bg-gray-700">
                    <div class="bg-white dark:bg-gray-800 p-4">
                        <span class="text-gray-900 dark:text-white font-semibold">Time</span>
                    </div>
                    @php
                    ...
                    @foreach($days as $day)
                        <div class="bg-white dark:bg-gray-800 p-4">
                            <span class="text-gray-900 dark:text-white font-semibold">{{ $day }}</span>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Mar {{ $loop->index + 15 }}</p>
                        </div>
                    @endforeach
                </div>
                ...
                @foreach($timeSlots as $time)
                    <div class="grid grid-cols-8 gap-px bg-gray-200 dark:bg-gray-700">
                        <div class="bg-white dark:bg-gray-800 p-4">
                            <span class="text-gray-900 dark:text-white">{{ $time }}</span>
                        </div>
                        @for($i = 0; $i < 7; $i++)
                            <div class="bg-white dark:bg-gray-800 p-4 min-h-[100px] relative group hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                                @if($loop->index === 1 && $i === 2)
                                    <div class="absolute inset-1 bg-blue-100 dark:bg-blue-900 rounded p-2">
                                        <p class="text-sm font-medium text-blue-700 dark:text-blue-300">John Doe</p>
                                        <p class="text-xs text-blue-600 dark:text-blue-400">Teeth Cleaning</p>
                                    </div>
                                @endif
                                @if($loop->index === 3 && $i === 4)
                                    <div class="absolute inset-1 bg-green-100 dark:bg-green-900 rounded p-2">
                                        <p class="text-sm font-medium text-green-700 dark:text-green-300">Jane Smith</p>
                                        <p class="text-xs text-green-600 dark:text-green-400">Root Canal</p>
                                    </div>
                                @endif
                            </div>
                        @endfor
        ...
        <!-- JavaScript for Calendar Navigation -->
        <script>
